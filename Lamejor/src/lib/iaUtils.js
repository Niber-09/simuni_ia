...
export const clasificarTramiteIA = (descripcion, tipoTramiteManual = null) => {
  ...
  if (texto.includes("urgente") || texto.includes("inmediato")) {
    if (prioridadAsignada === 'baja') {
      ...
      diasEstimados = categorias[tipoDetectado]?.prioridad === 'media' ? categorias[tipoDetectado].dias : '7-10 días'; 
    } else if (prioridadAsignada === 'media') {
      prioridadAsignada = 'alta';
      diasEstimados = categorias[tipoDetectado]?.prioridad === 'alta' ? categorias[tipoDetectado].dias : '5-7 días'; 
    }
  }
...
export const getPrioridadColor = (prioridad) => {
  switch (prioridad) {
    ...
    case 'media': return 'bg-yellow-500 hover:bg-yellow-600'; // Revertido a amarillo
